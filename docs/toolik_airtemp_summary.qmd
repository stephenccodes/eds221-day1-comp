...
```{r}
toolik_temp_plot <- ggplot(data = toolik_weather,
...
```{r}
toolik_chloryphyll <- read_csv(here::here("data", "data-raw", "toolik_chlorophyll.csv"))
```

```{r}
skimr::skim(toolik_chloryphyll)
```

```{r}
toolik_chloryphyll <- toolik_chloryphyll %>%
  clean_names()
```

```{r}
dim(toolik_chloryphyll)
names(toolik_chloryphyll)
```

```{r}
toolik_chloryphyll$active_chl_a_ug_l <-
na_if(toolik_chloryphyll$active_chl_a_ug_l, ".")

toolik_chloryphyll$active_chl_a_ug_l <- as.numeric(toolik_chloryphyll$active_chl_a_ug_l)
toolik_chloryphyll
```

```{r}
ggplot(data = toolik_chloryphyll, 
      aes( x = depth, y = active_chl_a_ug_l)) +
  geom_point(aes(color = active_chl_a_ug_l), 
             show.legend = FALSE) +
  scale_color_gradient(low = "blue", high = "orange")+
  theme_minimal()+
  labs(title = "Toolik Choryphyll Concentrations", 
       x = "Lake Depth (meters)", 
       y = "Active Chloryphyll Concentration (micrograms/liter)")
```
